...
cdef bytearray pixels_cython(int y, int n):
    cdef bytearray range7 = bytearray(range(7))
    cdef bytearray pixel_bits = bytearray(128 >> pos for pos in range(8))
    cdef double c1 = 2.0 / float(n)
    cdef double complex c0 = -1.5 + 1j * y * c1 - 1j
    cdef int x = 0
    cdef int pixel
