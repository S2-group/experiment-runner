...
class WattsUp(object):
    ...
    def sysCheck(self, port, timeout, outfile):           # Set up & check serial ports 
        if not port:
            system = uname()[0]
        ...
        if not os.path.exists(port):
                ...
                exit()
        self.log(timeout, outfile)

def run_meter(port, interval, timeout, outfile):
    meter = WattsUp(port, interval)            # init (Serial port, interval) *ttyUSB0 Linux default
    meter.sysCheck(port, timeout, outfile)     # run experiment (Serial port, timeout, export file)
